...
require.config({
	...
	// We shim Backbone.js and Underscore.js since they don't declare AMD
	// modules
	shim : {
		'backbone' : {
...
define("configuration",
		function() {
			if (window.TicketMonster != undefined
					&& TicketMonster.config != undefined) {
				return {
...
define(
		...
		function($, utilities, MainTemplate) {
			// Configure jQuery to append timestamps to requests, to bypass
			// browser caches
			// Important for MSIE
			$.ajaxSetup({
				cache : false
			...
			$(document).bind("mobileinit", function() {
				// Prior to creating and starting the router, we disable jQuery
				// Mobile's own routing mechanism
				$.mobile.hashListeningEnabled = false;
				$.mobile.linkBindingEnabled = false;
				$.mobile.pushStateEnabled = false;
				utilities.applyTemplate($('body'), MainTemplate);
			});
			// Then (load jQueryMobile and) start the router to finally start
			// the app
			require([ 'router' ]);
		});
